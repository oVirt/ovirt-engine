...
import static org.ovirt.engine.core.common.config.ConfigValues.VmSlaPolicySupported;
...
public class VmSlaPolicyCommandTest {
    ...
    public static MockConfigRule mockConfig = new MockConfigRule(
            MockConfigRule.mockConfig(VmSlaPolicySupported, Version.v3_6.getValue(), true)
    );
    ...
    public void setUp() {
        ...
        command = spy(new VmSlaPolicyCommand<>(parameters));
        ...
        Answer<?> answer = new Answer<Object>() {
            ...
            public Object answer(InvocationOnMock invocation) throws Throwable {
                ...
                retVal.setSucceeded(vdsFunction.eval(params));
                return retVal;
            }
        };
        ...
        vm.setVdsGroupCompatibilityVersion(Version.v3_6);

        when(vmDao.get(VM_ID)).thenReturn(vm);
        when(vmNetworkInterfaceDao.getAllForVm(any(Guid.class))).thenReturn(new ArrayList<VmNetworkInterface>());
    ...
    public void testVmDown() {
        ...
        CanDoActionTestUtils.runAndAssertCanDoActionFailure(command, EngineMessage.ACTION_TYPE_FAILED_VM_STATUS_ILLEGAL);
    }

    @Test
    public void testEmptyParams() {
        parameters.setCpuQos(null);
        parameters.setStorageQos(new HashMap<DiskImage, StorageQos>());
        CanDoActionTestUtils.runAndAssertCanDoActionFailure(command, EngineMessage.VM_SLA_POLICY_UNCHANGED);
    }
    ...
    public void testCpuQos() {
        ...
        vdsFunction = new Function<UpdateVmPolicyVDSParams, Boolean>() {
            @Override
            public Boolean eval(UpdateVmPolicyVDSParams params) {
    ...
    public void testStorageQos() {
        ...
        vdsFunction = new Function<UpdateVmPolicyVDSParams, Boolean>() {
            @Override
            public Boolean eval(UpdateVmPolicyVDSParams params) {
                VmSlaPolicyCommandTest.this.assertIoTune(params.getIoTuneList().get(0),
                        100L * 1024L * 1024L,
                        0,
                        0,
                        60000,
    ...
    public void testStorageQosUnlimited() {
        ...
        vdsFunction = new Function<UpdateVmPolicyVDSParams, Boolean>() {
            @Override
            public Boolean eval(UpdateVmPolicyVDSParams params) {
                VmSlaPolicyCommandTest.this.assertIoTune(params.getIoTuneList().get(0), 0, 0, 0, 0, 0, 0);
                return true;
            }
        };
