...
public class NetworkClusterHelper {
    ...
    private static boolean atLeastOneHostDoesNotHaveNetworkAttached(List<VDS> hosts, String networkName) {
        for (VDS host : hosts) {
            ...
            if (!hostHasInterfaceWithGivenNetwork(networkName, hostInterfaces)) {
                return true;
            }
        }
        return false;
    ...
    private static boolean hostHasInterfaceWithGivenNetwork(String networkName,
            List<VdsNetworkInterface> hostInterfaces) {

        for (VdsNetworkInterface hostInterface : hostInterfaces) {
            if (StringUtils.equals(hostInterface.getNetworkName(), networkName)) {
                return true;
            }
        }
    ...
    private static List<VDS> getHostsWithUpStatus(List<VDS> hosts) {
        List<VDS> hostsWhichAreUp = new ArrayList<>();
        for (VDS host : hosts) {
            if (host.getStatus() == VDSStatus.Up) {
                hostsWhichAreUp.add(host);
            }
        }

        return hostsWhichAreUp;
    }
