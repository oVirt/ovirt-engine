...
public class LibvirtSecretValidator {
    ...
    public ValidationResult uuidNotEmpty() {
        return ValidationResult.failWith(VdcBllMessages.LIBVIRT_SECRET_UUID_CANNOT_BE_EMPTY)
                .when(StringUtils.isEmpty(libvirtSecret.getId().toString()));
    }

    public ValidationResult uuidNotExist() {
        return ValidationResult.failWith(VdcBllMessages.LIBVIRT_SECRET_UUID_ALREADY_EXISTS)
                .unless(libvirtSecretDao.get(libvirtSecret.getId()) == null);
    }

    public ValidationResult uuidExist() {
        return ValidationResult.failWith(VdcBllMessages.LIBVIRT_SECRET_UUID_NOT_EXISTS)
                .unless(libvirtSecretDao.get(libvirtSecret.getId()) != null);
    }

    public ValidationResult valueNotEmpty() {
        return ValidationResult.failWith(VdcBllMessages.LIBVIRT_SECRET_VALUE_CANNOT_BE_EMPTY)
