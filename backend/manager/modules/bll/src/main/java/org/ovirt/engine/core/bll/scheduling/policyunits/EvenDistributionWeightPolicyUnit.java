...
public abstract class EvenDistributionWeightPolicyUnit extends PolicyUnitImpl {
    ...
    public List<Pair<Guid, Integer>> reverseEvenDistributionScore(VDSGroup cluster,
            List<VDS> hosts,
            VM vm,
    ...
    protected float getMaxMemoryOfVdsInCluster(List<VDS> hosts) {
        ...
        float maxMemoryOfVds = 0;
        for (VDS host : hosts) {
            if (host.getFreeVirtualMemory() > maxMemoryOfVds) {
                maxMemoryOfVds = host.getFreeVirtualMemory();
            }
        }
        return maxMemoryOfVds;
    }
