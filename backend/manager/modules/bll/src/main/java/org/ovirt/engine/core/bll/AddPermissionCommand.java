...
public class AddPermissionCommand<T extends PermissionsOperationsParameters> extends PermissionsCommandBase<T> {
     ...
     *
     * The identifier is matched first, because it names the row unambiguously. Name and domain are kept as a fallback
     * for principals backed by an external identity provider, but cannot stand alone: they resolve nothing for a
     * principal that carries no domain, such as the built-in Everyone group, nor for a request that referenced the
     * principal by identifier only.
     */
