...
public class HybridBackupCommand<T extends VmBackupParameters> extends StartVmBackupCommand<T> implements SerialChildExecutingCommand {
    ...
    protected void executeCommand() {
        ...
        ActionReturnValue returnValue = runInternalAction(
                ...
                createStepsContext(StepEnum.CREATING_SNAPSHOTS, getJobMessageProperties()));
    ...
    public boolean performNextOperation(int completedChildCount) {
        ...
        if (phase == VmBackupPhase.READY) {
            if (getParameters().getVmBackup().isStopped()) {
                Map<String, String> jobMessageProperties = getMergeSnapshotsJobMessageProperties();
                removeAutoGeneratedSnapshot(
                        createStepsContext(StepEnum.MERGE_SNAPSHOTS, jobMessageProperties),
                        getActionType(),
                        getParameters());
    ...
    private Map<String, String> getMergeSnapshotsJobMessageProperties() {
        Map<String, String> jobMessageProperties = getJobMessageProperties();
        String diskList = getDisks().stream().map(Disk::getDiskAlias).collect(Collectors.joining(", "));
        if (!diskList.isEmpty()) {
            jobMessageProperties.put(VdcObjectType.Disk.name().toLowerCase(), diskList);
        }
        String autogenSnapshotDescription = Optional.ofNullable(snapshotDao.get(getParameters().getAutoGeneratedSnapshotId()))
                .map(Snapshot::getDescription)
                .orElse("");
        String sourceSnapshotDescription = Optional.ofNullable(snapshotDao.get(getVmId(), SnapshotType.ACTIVE))
                .map(Snapshot::getDescription)
                .orElse("");
        jobMessageProperties.put("sourcesnapshot", autogenSnapshotDescription);
        jobMessageProperties.put("destinationsnapshot", sourceSnapshotDescription);
