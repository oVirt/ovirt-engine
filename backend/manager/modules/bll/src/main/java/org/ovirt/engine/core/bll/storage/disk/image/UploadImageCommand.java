...
public abstract class UploadImageCommand<T extends UploadImageParameters> extends BaseImagesCommand<T> {
    ...
    private void handleInitializing(final StateContext context) {
        ...
        case NOT_STARTED:
        case ACTIVE:
            if(getParameters().getVdsmTaskIds().size() > 0
                    && CommandCoordinatorUtil.getAsyncTaskFromDb(getParameters().getVdsmTaskIds().get(0)) == null) {
                CommandCoordinatorUtil.updateCommandStatus(context.childCmdId, CommandStatus.SUCCEEDED);
                break;
            }
