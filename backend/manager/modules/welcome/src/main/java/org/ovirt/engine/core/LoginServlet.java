...
public class LoginServlet extends HttpServlet {
    ...
    protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (deployedResponse.containsKey(WelcomeUtils.ERROR)) {
            ...
            response.sendRedirect(EngineLocalConfig.getInstance().getProperty(WelcomeUtils.ENGINE_URI) + WelcomeUtils.ERROR_PAGE_URI);
        } else {
            response.sendRedirect(new URLBuilder(EngineLocalConfig.getInstance().getProperty(WelcomeUtils.ENGINE_SSO_AUTH_URL), WelcomeUtils.OAUTH_AUTHORIZE_URI)
                    .addParameter(WelcomeUtils.HTTP_PARAM_CLIENT_ID, EngineLocalConfig.getInstance().getProperty(WelcomeUtils.ENGINE_SSO_CLIENT_ID))
                    ...
                    .addParameter(WelcomeUtils.HTTP_PARAM_STATE, state).build());
        }
    }
