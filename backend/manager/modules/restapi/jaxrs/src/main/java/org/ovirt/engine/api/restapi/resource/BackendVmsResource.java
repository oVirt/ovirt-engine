...
public class BackendVmsResource extends
    ...
    private Response cloneVmFromTemplate(VmStatic staticVm, VM vm, VmTemplate template, InstanceType instanceType, VDSGroup cluster) {
        AddVmFromTemplateParameters params = new AddVmFromTemplateParameters(staticVm, getDisksToClone(vm.getDisks(), template.getId()), Guid.Empty);
    ...
    private void addDevicesToParams(VmManagementParametersBase params, VM vm, VmTemplate template, InstanceType instanceType, int osId, VDSGroup cluster) {
        ...
        if (vm.isSetVirtioScsi()) {
            ...
            params.setVirtioScsiEnabled(instanceTypeId != null ? !VmHelper.getInstance().getVirtioScsiControllersForEntity(instanceTypeId).isEmpty() : null);
        }

        if(vm.isSetSoundcardEnabled()) {
            params.setSoundDeviceEnabled(vm.isSoundcardEnabled());
        } else if (instanceTypeId != null || templateId != null) {
            params.setSoundDeviceEnabled(!VmHelper.getInstance().getSoundDevicesForEntity(instanceTypeId != null ? instanceTypeId : templateId).isEmpty());
        }
        ...
        if (vm.isSetMemoryPolicy()) {
            ...
            // it is not defined on the template
            params.setBalloonEnabled(instanceTypeId != null ? !VmHelper.getInstance().isMemoryBalloonEnabledForEntity(instanceTypeId) : null);
        }
        ...
        if (vm.isSetRngDevice()) {
            ...
            List<VmRngDevice> devices = VmHelper.getInstance().getRngDevicesForEntity(instanceTypeId != null ? instanceTypeId : templateId);
