...
public class VmsMonitoringTest {
    ...
    public void updateVmDevicesNull() {
        vmsMonitoring.updateVmDevices(Collections.singletonList(""));

        assertEquals("wrong number of new devices", 0, vmsMonitoring.getNewVmDevices().size());
    ...
    public void updateLunDisksNoMismatch() {
        ...
        assertEquals("wrong number of LUNs to update", 0, vmsMonitoring.getVmLunDisksToSave().size());
    }
    ...
    public void updateLunDisksMismatch() {
        ...
        assertEquals("wrong number of LUNs to update", 1, vmsMonitoring.getVmLunDisksToSave().size());
    }
    ...
    private void updateLunDisksTest(Map<String, LUNs> lunsMapFromVmStats, List<Disk> vmLunDisksFromDb) {
        ...
        when(vmsMonitoring.getRunningVms()).thenReturn(Collections.singletonMap(vmId, vmInternalData));
        poweringUpVms = Collections.singletonList(vmDynamic);

        vmsMonitoring.updateLunDisks();
    }
    ...
    public void isNewWatchdogEvent() {
        ...
        VM vm = new VM();
        assertFalse(VmsMonitoring.isNewWatchdogEvent(dynamic, vm));
        dynamic.setLastWatchdogEvent(1L);
        assertTrue(VmsMonitoring.isNewWatchdogEvent(dynamic, vm));
    ...
    public void verifyListOfRunningVmsIsSameWithSuccessFromVdsmResponse() {
        ...
        // start refreshing vm data... VmsMonitoring now fetches Vms list from ResourceManager and loop through it
        vmsMonitoring.fetchRunningVms();
        List<Guid> staleRunningVms = vmsMonitoring.checkVmsStatusChanged();

        Assert.assertTrue(vmsMonitoring.getRunningVms().containsKey(VM_1));
    ...
    public void verifyListOfRunningVmsIsSameWithFailureOnGetVmStats() {
        ...
        Assert.assertFalse(vmsMonitoring.getRunningVms().containsKey(VM_1));
    ...
    private void mockVmsMonitoring() {
