...
Create or replace FUNCTION Insertstateless_vm_image_map(v_image_guid UUID,
	v_internal_drive_mapping VARCHAR(50) ,
	v_vm_guid UUID)
RETURNS VOID
   AS $procedure$
BEGIN
INSERT INTO stateless_vm_image_map(image_guid, internal_drive_mapping, vm_guid)
...
Create or replace FUNCTION Updatestateless_vm_image_map(v_image_guid UUID,
	v_internal_drive_mapping VARCHAR(50) ,
	v_vm_guid UUID)
RETURNS VOID

	--The [stateless_vm_image_map] table doesn't have a timestamp column. Optimistic concurrency logic cannot be generated
   AS $procedure$
BEGIN
      UPDATE stateless_vm_image_map
...
Create or replace FUNCTION Deletestateless_vm_image_map(v_image_guid UUID)
RETURNS VOID
   AS $procedure$
BEGIN
	
   DELETE FROM stateless_vm_image_map
   WHERE image_guid = v_image_guid;
...
Create or replace FUNCTION GetAllFromstateless_vm_image_map() RETURNS SETOF stateless_vm_image_map
   AS $procedure$
BEGIN
RETURN QUERY SELECT stateless_vm_image_map.*
   FROM stateless_vm_image_map;
...
Create or replace FUNCTION Getstateless_vm_image_mapByimage_guid(v_image_guid UUID) RETURNS SETOF stateless_vm_image_map
   AS $procedure$
BEGIN
RETURN QUERY SELECT stateless_vm_image_map.*
   FROM stateless_vm_image_map
...
Create or replace FUNCTION Getstateless_vm_image_mapByvm_guid(v_vm_guid UUID) RETURNS SETOF stateless_vm_image_map
   AS $procedure$
BEGIN
RETURN QUERY SELECT stateless_vm_image_map.*
   FROM stateless_vm_image_map
...
Create or replace FUNCTION GetVmByImageId(v_image_guid UUID) RETURNS SETOF vms
   AS $procedure$
BEGIN
      RETURN QUERY SELECT DISTINCT vms.*
      FROM vms
