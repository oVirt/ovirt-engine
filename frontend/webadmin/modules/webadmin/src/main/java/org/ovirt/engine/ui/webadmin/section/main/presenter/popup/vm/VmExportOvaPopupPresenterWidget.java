package org.ovirt.engine.ui.webadmin.section.main.presenter.popup.vm;

import org.ovirt.engine.ui.common.presenter.AbstractModelBoundPopupPresenterWidget;
import org.ovirt.engine.ui.uicommonweb.models.vms.ExportOvaModel;

import com.google.gwt.event.shared.EventBus;
import com.google.inject.Inject;

public class VmExportOvaPopupPresenterWidget extends AbstractModelBoundPopupPresenterWidget<ExportOvaModel, VmExportOvaPopupPresenterWidget.ViewDef> {

    public interface ViewDef extends AbstractModelBoundPopupPresenterWidget.ViewDef<ExportOvaModel> {
    }

    @Inject
    public VmExportOvaPopupPresenterWidget(EventBus eventBus, ViewDef view) {
        super(eventBus, view);
    }
}
