    ...
    public void init(final GlusterVolumeGeoRepCreateModel model) {
        ...
        model.getPropertyChangedEvent().addListener(new IEventListener() {
            ...
            public void eventRaised(Event ev, Object sender, EventArgs args) {
                if(((PropertyChangedEventArgs)args).propertyName.equalsIgnoreCase("RecommendationViolations")) {//$NON-NLS-1$
                    getView().setSuggestedConfigViolations(model.getRecommendationViolations());
                } else if (((PropertyChangedEventArgs)args).propertyName.equalsIgnoreCase("QueryFailed")) {//$NON-NLS-1$
