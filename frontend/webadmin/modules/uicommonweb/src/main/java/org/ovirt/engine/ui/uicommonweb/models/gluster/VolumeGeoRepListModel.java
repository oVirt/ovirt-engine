...
public class VolumeGeoRepListModel extends SearchableListModel{
    ...
    private void populateStatus(final List<GlusterGeoRepSessionDetails> details) {
        ...
        final List<EntityModel<GlusterGeoRepSessionDetails>> detailRows = new ArrayList<EntityModel<GlusterGeoRepSessionDetails>>();
            ...
            public void onSuccess(Object model, Object returnValue) {
                ...
                if (!queryExecutionStatus) {
                    ...
                    List<EntityModel<Pair<Boolean, GlusterGeoRepSessionConfiguration>>> sessionConfigEntities =
                            new ArrayList<EntityModel<Pair<Boolean, GlusterGeoRepSessionConfiguration>>>();
                            for (GlusterGeoRepSessionConfiguration currentSession : sessionConfigs) {
                                sessionConfigEntities.add(new EntityModel<Pair<Boolean, GlusterGeoRepSessionConfiguration>>(new Pair<Boolean, GlusterGeoRepSessionConfiguration>(false,
                                        currentSession)));
                            }
                    ...
                    public void executed(FrontendActionAsyncResult result) {
                        ...
                        if (result.getReturnValue().getSucceeded()) {
                            ...
                            if (createModel.getStartSession().getEntity()) {
                                initializeGeoRepActionConfirmation(constants.geoReplicationStartTitle(), HelpTag.volume_geo_rep_start_confirmation, "volume_geo_rep_start_confirmation", constants.geoRepForceHelp(), messages.geoRepForceTitle(constants.startGeoRep()), "onStartGeoRepSession", getEntity().getName(), remoteVolumeName, remoteHostName);//$NON-NLS-1$//$NON-NLS-2$
