...
public class UserPortalListModel extends IUserPortalListModel implements IVmPoolResolutionService
{
	...
	protected void SyncSearch()
	{
		...
		Frontend.RunQuery(VdcQueryType.GetUserVmsByUserIdAndGroups, new GetUserVmsByUserIdAndGroupsParameters(Frontend.getLoggedInUser().getUserId()), _asyncQuery);

		AsyncQuery _asyncQuery1 = new AsyncQuery();
		_asyncQuery1.setModel(this);
		_asyncQuery1.asyncCallback = new INewAsyncCallback() { public void OnSuccess(Object model, Object ReturnValue)
		{
			UserPortalListModel userPortalListModel = (UserPortalListModel)model;
		  ...
		  Frontend.RunQuery(VdcQueryType.GetAllVmPoolsAttachedToUser, new GetAllVmPoolsAttachedToUserParameters(Frontend.getLoggedInUser().getUserId()), _asyncQuery1);
		...
		new INewAsyncCallback() {
			...
			public void OnSuccess(Object target, Object returnValue) {
				...
				if (!isNameUnique)
				{
					...
					userPortalListModel.getVmModel().getName().getInvalidityReasons().add("Name must be unique.");
	...
	private void RunOnce()
	{
		...
		_asyncQuery0.asyncCallback = new INewAsyncCallback() { public void OnSuccess(Object model0, Object result0)
		{
			if (result0 != null)
			{
				...
				_asyncQuery01.asyncCallback = new INewAsyncCallback() { public void OnSuccess(Object model1, Object result)
				{
				...
				_asyncQuery02.asyncCallback = new INewAsyncCallback() { public void OnSuccess(Object model2, Object result)
				{
					UserPortalListModel userPortalListModel = (UserPortalListModel)model2;
					UserPortalItemModel userPortalItemModel = (UserPortalItemModel)userPortalListModel.getSelectedItem();
					RunOnceModel runOnceModel = userPortalListModel.getRunOnceModel();
		...
		_asyncQuery2.asyncCallback = new INewAsyncCallback() { public void OnSuccess(Object model3, Object ReturnValue)
		{
	...
	private void UpdateActionAvailability()
	{
		...
		getRemoveCommand().setIsExecutionAllowed(selectedItem != null && !selectedItem.getIsPool() && VdcActionUtils.CanExecute(new java.util.ArrayList<VM>(java.util.Arrays.asList(new VM[] { (VM)selectedItem.getEntity() })), VM.class, VdcActionType.RemoveVm));

		getRunOnceCommand().setIsExecutionAllowed(selectedItem != null && !selectedItem.getIsPool() && VdcActionUtils.CanExecute(new java.util.ArrayList<VM>(java.util.Arrays.asList(new VM[] { (VM)selectedItem.getEntity() })), VM.class, VdcActionType.RunVmOnce));

		getChangeCdCommand().setIsExecutionAllowed(selectedItem != null && !selectedItem.getIsPool() && VdcActionUtils.CanExecute(new java.util.ArrayList<VM>(java.util.Arrays.asList(new VM[] { (VM)selectedItem.getEntity() })), VM.class, VdcActionType.ChangeDisk));

		getNewTemplateCommand().setIsExecutionAllowed(selectedItem != null && !selectedItem.getIsPool() && VdcActionUtils.CanExecute(new java.util.ArrayList<VM>(java.util.Arrays.asList(new VM[] { (VM)selectedItem.getEntity() })), VM.class, VdcActionType.AddVmTemplate));
	}
		...
		_asyncQuery0.asyncCallback = new INewAsyncCallback() { public void OnSuccess(Object model0, Object result0)
			{
				...
				java.util.ArrayList<String> images0 = new java.util.ArrayList<String>(java.util.Arrays.asList(new String[] { "No CDs" }));
				...
				if (result0 != null)
				{
					...
					_asyncQuery.asyncCallback = new INewAsyncCallback() { public void OnSuccess(Object model1, Object result)
						{
							UserPortalListModel userPortalListModel = (UserPortalListModel)model1;
	...
	public void PostVmNameUniqueCheck(UserPortalListModel userPortalListModel)
	{
		...
		gettempVm().setcpu_per_socket((Integer)getVmModel().getTotalCPUCores().getEntity() / (Integer)getVmModel().getNumOfSockets().getEntity());
		...
		gettempVm().setiso_path(getVmModel().getCdImage().getIsChangable() ? (String)getVmModel().getCdImage().getSelectedItem() : "");
	...
	private void VmModel_DataCenter_ItemsChanged()
	{
		...
		if (!getVmModel().getIsNew() && dataCenter == null)
		{
			...
			_asyncQuery.asyncCallback = new INewAsyncCallback() { public void OnSuccess(Object model, Object result)
											{
												UserPortalListModel userPortalListModel = (UserPortalListModel)model;
												java.util.ArrayList<storage_pool> list = new java.util.ArrayList<storage_pool>(java.util.Arrays.asList(new storage_pool[] { (storage_pool)result }));
	...
	private void VmModel_TimeZone_ItemsChanged()
	{
		if (!getVmModel().getIsNew())
		{
			...
			{
				getVmModel().getTimeZone().setSelectedItem(Linq.FirstOrDefault((Iterable<java.util.Map.Entry<String, String>>)getVmModel().getTimeZone().getItems(), new Linq.TimeZonePredicate(vm.gettime_zone())));
			}
		}
	}
	...
	public void OnVmAndPoolLoad()
	{
		if (getvms() != null && getpools() != null)
		{
			...
			setCanConnectAutomatically(GetUpVms(items).size() == 1 && ((UICommand)GetUpVms(items).get(0).getDefaultConsole().getConnectCommand()).getIsExecutionAllowed());
