	...
	public void Edit()
	{
		...
		_asyncQuery.asyncCallback = new INewAsyncCallback() { public void OnSuccess(Object model, Object ReturnValue)
		{
			...
			else
			{
				...
				java.util.ArrayList<VdsNetworkInterface> selectedItems = hostInterfaceListModel.getSelectedItemsWithVlans();
...
//C# TO JAVA CONVERTER TODO TASK: Comparisons involving nullable type instances are not converted to null-value logic:
	...
	public void EditManagementNetwork()
	{
		...
		_asyncQuery.asyncCallback = new INewAsyncCallback() { public void OnSuccess(Object model, Object ReturnValue)
		{
			...
			java.util.ArrayList<VdsNetworkInterface> interfaces = DataProvider.GetInterfaceOptionsForEditNetwork(getOriginalItems(), item, networkToEdit, getEntity().getvds_id(), tempRef_defaultInterfaceName);
...
//C# TO JAVA CONVERTER TODO TASK: Comparisons involving nullable type instances are not converted to null-value logic:
			...
			managementModel.getCheckConnectivity().setEntity(item.getIsManagement()); // currently, always should be true
...
//C# TO JAVA CONVERTER TODO TASK: Comparisons involving nullable type instances are not converted to null-value logic:
	...
	public void OnEditManagementNetwork()
	{
		...
		new IFrontendActionAsyncCallback() {
			...
			{
				EntityModel commitChanges = ((HostManagementNetworkModel)hostInterfaceListModel.getcurrentModel()).getCommitChanges();
	...
	public void Bond()
	{
		...
		_asyncQuery.asyncCallback = new INewAsyncCallback() { public void OnSuccess(Object model, Object ReturnValue)
		{
			...
			RefObject<Boolean> tempRef_isVlanSelected = new RefObject<Boolean>(isVlanSelected);
			RefObject<Boolean> tempRef_isManagement = new RefObject<Boolean>(isManagement);
			java.util.ArrayList<String> selectedNicsNetworks = hostInterfaceListModel.GetSelectedNicsNetworks(tempRef_isVlanSelected, tempRef_isManagement);
			isVlanSelected = tempRef_isVlanSelected.argvalue;
			isManagement = tempRef_isManagement.argvalue;
			...
			{
				innerBondModel.getBond().setItems(new java.util.ArrayList<VdsNetworkInterface>(java.util.Arrays.asList(new VdsNetworkInterface[] { bond })));
				...
				_asyncQuery1.asyncCallback = new INewAsyncCallback() { public void OnSuccess(Object model1, Object ReturnValue1)
					{
						...
						innerHostInterfaceListModel.PostBond(innerHostInterfaceListModel, bModel, bModel.getNetwork().getItems() != null ? (java.util.ArrayList<network>)bModel.getNetwork().getItems() : new java.util.ArrayList<network>(), hasManagement);

					}};
				Frontend.RunQuery(VdcQueryType.GetVdsFreeBondsByVdsId, new GetVdsByVdsIdParameters(host.getvds_id()),_asyncQuery1);
			}
		}};
		AsyncDataProvider.GetClusterNetworkList(_asyncQuery, getEntity().getvds_group_id());
	}

	public void PostBond(HostInterfaceListModel hostInterfaceListModel, HostBondInterfaceModel innerBondModel, java.util.ArrayList<network> networksToAdd, boolean isAnyManagement)
	{
	...
	public void OnBond()
	{
		...
		new IFrontendActionAsyncCallback() {
			...
			public void Executed(FrontendActionAsyncResult  result) {
				...
				if (returnValueBase != null && returnValueBase.getSucceeded())
				{
					EntityModel commitChanges = ((HostBondInterfaceModel)hostInterfaceListModel.getcurrentModel()).getCommitChanges();
					if ((Boolean)commitChanges.getEntity())
					{
						SaveNetworkConfig(hostInterfaceListModel.getEntity().getvds_id(), hostInterfaceListModel);
					}
