...
public class VmModel extends Model
{
	...
	private void UpdateDomain()
	{
		...
		new INewAsyncCallback() {
			...
			vmModel.getDomain().setSelectedItem((oldDomain != null) ? oldDomain : Linq.FirstOrDefault(domains));
	...
	private void UpdateTotalCpus()
	{
		...
		int totalCPUCores = getTotalCPUCores().getEntity() != null ? Integer.parseInt(getTotalCPUCores().getEntity().toString()) : 0;
		...
		if ((totalCPUCores % numOfSockets != 0) && totalCPUCores < getTotalCPUCores().getMax() && totalCPUCores > getTotalCPUCores().getMin())
		{
	...
	private void DataCenter_SelectedItemChanged()
	{
		...
		java.util.ArrayList<VDSGroup> clusters = dataCenter != null ? DataProvider.GetClusterList(dataCenter.getId()) : DataProvider.GetClusterList();
	...
	private void Template_SelectedItemChanged()
	{
		...
		setIsBlankTemplate(template.getId().equals(Guid.Empty));
		...
		getTimeZone().setSelectedItem(StringHelper.isNullOrEmpty(template.gettime_zone()) ? Linq.FirstOrDefault((Iterable<java.util.Map.Entry<String, String>>)getTimeZone().getItems(), new Linq.TimeZonePredicate(DataProvider.GetDefaultTimeZone())) : Linq.FirstOrDefault((Iterable<java.util.Map.Entry<String, String>>)getTimeZone().getItems(), new Linq.TimeZonePredicate(template.gettime_zone())));
	...
	private void Cluster_SelectedItemChanged()
	{
		...
		java.util.ArrayList<VDS> hosts = cluster != null ? DataProvider.GetHostListByCluster(cluster.getname()) : new java.util.ArrayList<VDS>();
		...
		if (cluster != null)
		{
			...
			setIsCustomPropertiesAvailable(DataProvider.IsSupportCustomProperties(cluster.getcompatibility_version().toString()));
		}
	...
	private void OSType_SelectedItemChanged()
	{
		...
		setIsLinux_Unassign_UnknownOS(DataProvider.IsLinuxOsType((VmOsType)getOSType().getSelectedItem()) || ((VmOsType)getOSType().getSelectedItem()) == VmOsType.Unassigned || ((VmOsType)getOSType().getSelectedItem()) == VmOsType.Other);
	...
	public boolean Validate()
	{
		...
		String nameMsg = StringFormat.format("Name must contain only alphanumeric characters. Maximum length: %1$s.", WINDOWS_VM_NAME_MAX_LIMIT);

		if (!DataProvider.IsWindowsOsType(os))
		{
			nameExpr = StringFormat.format("^[-\\w]{1,%1$s}$", NON_WINDOWS_VM_NAME_MAX_LIMIT);
			nameMsg = StringFormat.format("Name cannot contain blanks or special characters. Maximum length: %1$s.", NON_WINDOWS_VM_NAME_MAX_LIMIT);
		}
		...
		if (this.isLinux_Unassign_UnknownOS && ((((String)getKernel_parameters().getEntity()).length() > 0 || ((String)getInitrd_path().getEntity()).length() > 0) && ((String)getKernel_path().getEntity()).length() == 0))
		{
			int count = 0;
			String msg = "When ";
	...
	private void ValidateMemorySize(EntityModel memorySizeEntityModel, int maxMemSize, int minMemSize)
	{
		...
		if (memSize == 0)
		{
			...
			memorySizeEntityModel.getInvalidityReasons().add(StringFormat.format("Memory size is between %1$s MB and %2$s MB", minMemSize, maxMemSize));
		}
		else if (memSize > maxMemSize)
		{
			memorySizeEntityModel.setIsValid(false);
			memorySizeEntityModel.getInvalidityReasons().add(StringFormat.format("Maximum memory size is %1$s MB.", maxMemSize));
		}
		else if (memSize < minMemSize)
		{
			memorySizeEntityModel.setIsValid(false);
			memorySizeEntityModel.getInvalidityReasons().add(StringFormat.format("Minimum memory size is %1$s MB.", minMemSize));
		}
	}
	...
	public BootSequence getBootSequence()
	{
		...
		return BootSequence.valueOf(StringFormat.format("%1$s%2$s", firstSelectedItem.getEntity(), secondSelectedItem.getEntity()));
	}
	public void setBootSequence(BootSequence value)
	{
		...
		java.util.ArrayList<EntityModel> secondDeviceOptions = Linq.<EntityModel>Cast(getSecondBootDevice().getItems());
