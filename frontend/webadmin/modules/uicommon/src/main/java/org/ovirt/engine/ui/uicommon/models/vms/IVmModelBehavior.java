...
public abstract class IVmModelBehavior
{
	...
	public void PostUpdateCdImage(Guid storageDomainId)
	{
		...
		}, getModel().getHash()), storageDomainId, false);
	}
	...
	public void PostUpdateTimeZone()
	{
		...
		String oldTimeZoneKey = ((java.util.Map.Entry<String, String>)getModel().getTimeZone().getSelectedItem()).getKey();

		getModel().getTimeZone().setItems(cachedTimeZones);
		if (oldTimeZoneKey != null)
		{
			getModel().getTimeZone().setSelectedItem(Linq.FirstOrDefault(cachedTimeZones, new Linq.TimeZonePredicate(oldTimeZoneKey)));
		}
		else
		{
...
//C# TO JAVA CONVERTER TODO TASK: Arithmetic operations involving nullable type instances are not converted to null-value logic:
		...
		new INewAsyncCallback() {
			...
			if(model.getBehavior().getSystemTreeSelectedItem() != null && model.getBehavior().getSystemTreeSelectedItem().getType() == SystemTreeItemType.Host)
			{
				...
				for (VDS vds : hosts)
				{
					...
					{
						model.getDefaultHost().setItems(new java.util.ArrayList<VDS>(java.util.Arrays.asList(new VDS[] {vds})));
			...
			else
			{
				...
				model.getDefaultHost().setSelectedItem(oldDefaultHost != null ? Linq.FirstOrDefault(hosts, new Linq.HostPredicate(oldDefaultHost.getvds_id())) : Linq.FirstOrDefault(hosts));
			}
			ChangeDefualtHost();

			}
		}, getModel().getHash()), cluster.getname());
	}
	...
	public void UpdateTotalCpus()
	{
		...
		int totalCPUCores = getModel().getTotalCPUCores().getEntity() != null ? Integer.parseInt(getModel().getTotalCPUCores().getEntity().toString()) : 0;
		...
		if ((totalCPUCores % numOfSockets != 0) && totalCPUCores < getModel().getTotalCPUCores().getMax() && totalCPUCores > getModel().getTotalCPUCores().getMin())
		{
