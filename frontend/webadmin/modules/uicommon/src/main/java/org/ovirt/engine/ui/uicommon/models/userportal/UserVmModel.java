...
public class UserVmModel extends Model
{
		...
		_asyncQuery.asyncCallback = new INewAsyncCallback() { public void OnSuccess(Object model, Object result)
									  {
										  ...
										  {
											  oldSelectedItem = ((java.util.Map.Entry<String, String>)userVmModel.getTimeZone().getSelectedItem()).getKey();
										  }

			userVmModel.getTimeZone().setItems(((java.util.HashMap<String, String>)result).entrySet());

										  userVmModel.getTimeZone().setSelectedItem(!StringHelper.isNullOrEmpty(oldSelectedItem) ? Linq.FirstOrDefault((Iterable<java.util.Map.Entry<String, String>>)userVmModel.getTimeZone().getItems(), new Linq.TimeZonePredicate(oldSelectedItem)) : Linq.FirstOrDefault((Iterable<java.util.Map.Entry<String, String>>)userVmModel.getTimeZone().getItems()));
									  }};
		...
		if (getIsNew())
		{
			...
			if (StringHelper.isNullOrEmpty(template.gettime_zone()))
			{
				...
				_asyncQuery.asyncCallback = new INewAsyncCallback() { public void OnSuccess(Object model, Object result)
												{
													UserVmModel userVmModel = (UserVmModel)model;
													userVmModel.getTimeZone().setSelectedItem(Linq.FirstOrDefault((Iterable<java.util.Map.Entry<String, String>>)getTimeZone().getItems(), new Linq.TimeZonePredicate((String)result)));
												}};
				AsyncDataProvider.GetDefaultTimeZone(_asyncQuery);
			}
			else
			{
				getTimeZone().setSelectedItem(Linq.FirstOrDefault((Iterable<java.util.Map.Entry<String, String>>)getTimeZone().getItems(), new Linq.TimeZonePredicate(template.gettime_zone())));
			}
	...
	private void SetGUIByVMType()
	{
		...
		{
			java.util.ArrayList<Integer> numOfMonitors = new java.util.ArrayList<Integer>(java.util.Arrays.asList(new Integer[] { 1 }));
						...
						_asyncQuery.asyncCallback = new INewAsyncCallback() { public void OnSuccess(Object model, Object result)
							{
								...
								else if (userVmModel.getCdImage().getIsChangable() && userVmModel.getCdImage().getSelectedItem() == null)
								{
		...
		if (template != null && !template.getId().equals(Guid.Empty))
		{
			...
			if ((Boolean)((EntityModel)getProvisioning().getSelectedItem()).getEntity())
			{
				...
				_asyncQuery.asyncCallback = new INewAsyncCallback() { public void OnSuccess(Object model, Object result)
												{
													UserVmModel userVmModel = (UserVmModel)model;
													java.util.ArrayList<storage_domains> storageDomains = (java.util.ArrayList<storage_domains>)result;
	...
	private void ValidateMemorySize(EntityModel memorySizeEntityModel, int maxMemSize)
	{
		...
		if (memSize == 0)
		{
			...
			memorySizeEntityModel.getInvalidityReasons().add("Memory size is between " + _minMemSize + " MB and " + maxMemSize + " MB");
		}
