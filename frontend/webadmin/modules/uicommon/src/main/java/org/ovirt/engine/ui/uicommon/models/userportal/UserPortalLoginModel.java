...
public class UserPortalLoginModel extends LoginModel
{
	...
	public void Login()
	{
		...
		Frontend.RunAction(VdcActionType.LoginUser, new LoginUserParameters((String)getUserName().getEntity(), (String)getPassword().getEntity(), (String)getDomain().getSelectedItem(), "", "", ""),
	...
	{
		VdcReturnValueBase returnValue = Frontend.RunAction(VdcActionType.ChangeUserPassword, new ChangeUserPasswordParameters((String)getUserName().getEntity(), (String)getPassword().getEntity(), (String)getNewPassword().getEntity(), (String)getDomain().getSelectedItem()));
	...
	protected boolean Validate()
	{
		...
		if (getIsChangingPassword())
		{
			...
			if (!((String)getNewPassword().getEntity()).equals((String)getVerifyPassword().getEntity()))
			{
				getVerifyPassword().setIsValid(false);
				getVerifyPassword().getInvalidityReasons().add("TODO: Verify password field doesn't match a new password.");
			}
		}
	...
	public void GetUserRoles(Object targetObject)
	{
		...
		new INewAsyncCallback() {
			...
			for (permissions permission : permissions)
			{
				...
				{
					everyoneBlankPermission = permission.getad_element_id().getValue().equals(Everyone) && permission.getObjectId().getValue().equals(Blank) && permission.getrole_id().getValue().equals(UserTemplateBasedVM) && permission.getObjectType().equals(VdcObjectType.VmTemplate);
					if (everyoneBlankPermission)
					{
						continue;
					}
	...
	public void UpdateUserActionGroups(Object targetObject, java.util.ArrayList<Guid> roleIdList)
	{
		...
		new INewAsyncCallback() {
			...
			public void OnSuccess(Object target, Object returnValue) {
				...
				UserPortalLoginModel loginModel = (UserPortalLoginModel)target;
				java.util.ArrayList<ActionGroup> roleActionGroupList = (java.util.ArrayList<ActionGroup>)returnValue;
