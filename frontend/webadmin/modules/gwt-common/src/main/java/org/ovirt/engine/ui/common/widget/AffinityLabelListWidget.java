...
public class AffinityLabelListWidget extends AbstractItemListWidget<ListModel<Label>, Label> {
    ...
    protected void createItems() {
        ...
        for (Label label : selectedLabels) {
            final Label selectedLabel = label;
            ItemListItem labelListItem = new ItemListItem();
            labelListItem.init(selectedLabel.getName());
            labelListItem.getDeactivationAnchor().addClickHandler(new ClickHandler() {
                @Override
                public void onClick(ClickEvent event) {
                getModel().getSelectedItems().remove(selectedLabel);
                refreshItems();
                }
            });
