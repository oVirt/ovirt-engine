...
public class SelectBoxListModelBinded extends UPSelectItem implements ModelBindedComponent {
	private ListModel listModel;
	private int itemsCount = 0;
	
	boolean async;
	
	public SelectBoxListModelBinded(String title, ListModel model, Class<?> type) {
		this(true, title, model, ObjectNameIdResolverFactory.getResolver(type));
	}
	
	public SelectBoxListModelBinded(boolean async, String title, ListModel model, Class<?> type) {
		this(async, title, model, ObjectNameIdResolverFactory.getResolver(type));
	}
	
	public SelectBoxListModelBinded(String title, ListModel model, ObjectNameIdResolver resolver) {
		this(true, title, model, resolver);
	}

	public SelectBoxListModelBinded(boolean async, String title, ListModel model, ObjectNameIdResolver resolver) {
		...
		model.getPropertyChangedEvent().addListener(new IEventListener() {
			...
			public void eventRaised(Event ev, Object sender, EventArgs args) {
				...
				if (property.equals("IsChangable")) {
					...
					else if (itemsCount <= 1 && model.getIsChangable()) {
						setDisabled(false);
	...
	abstract class ChangedModelItemEventHandler implements IEventListener {
		...
		private SelectBoxListModelBinded selectBox;
		private Model model;
		public LinkedHashMap<String,String> valueMap; 

		public ChangedModelItemEventHandler(SelectBoxListModelBinded selectBox, ListModel model) {
		...
		public void eventRaised(Event ev, Object sender, EventArgs args) {
			...
			if (items != null) {
				...
				if (itemsCount <= 1 && model.getIsChangable()) {
					setShowDisabled(false);
					setDisabled(true);
				}
				else {
	...
	abstract class BoxSelectItemChanged implements ChangedHandler {
		...
		public void onChanged(ChangedEvent event) {
			Iterable items = model.getItems();
			for (Object o : items) {
				if (event.getValue().toString().equals(getItemId(o))) {
					GWT.log("Selected Item: " + o);
