...
The engine container uses the hostname `ovirt-engine.test`. To access the oVirt UI in your browser, add this hostname to your local hosts file, pointing to the IP address of the machine where the container runs:

Add a line in `/etc/hosts`:
```
192.168.1.100  ovirt-engine.test
```

Replace `192.168.1.100` with the actual IP of the machine running the container. If running locally, use `127.0.0.1`.
