...
- maven-3
- ansible >= 2.4
- ovirt-ansible-roles (more info in Host deploy via Ansible section)
...
== Host deploy via Ansible

Since version 4.2 host deploy process contains also invocation of
ovirt-host-deploy Ansible role. This role is not part of engine, but it's
a part of https://github.com/ovirt/ovirt-ansible[oVirt Ansible Roles] project.

In order to make it work clone the project and run:

  $ git clone git@github.com:oVirt/ovirt-ansible.git
  $ cd ovirt-ansible
  $ make install PREFIX="${PREFIX}"

It will install all needed playbooks and roles to your dev env.
If you don't install it, then host deploy process will fail.
