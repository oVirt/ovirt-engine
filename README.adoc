...
Since version 4.2 host deploy process configure firewalld. It was
decided to use Ansible for it. The roles to configure firewalld are
stored in different https://github.com/ovirt/ovirt-ansible[git project].
...
If you don't install it, the firewalld configuration for the host will
be skipped.
