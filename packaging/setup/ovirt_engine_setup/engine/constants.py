...
class FileLocations(object):
    ...
    OVIRT_ENGINE_SERVICE_CONFIG_DEFAULTS = config.ENGINE_SERVICE_CONFIG_DEFAULTS
    OVIRT_ENGINE_NOTIFIER_SERVICE_CONFIG = config.ENGINE_NOTIFIER_SERVICE_CONFIG
    ...
    OVIRT_ENGINE_DB_UTILS_DIR = os.path.join(OVIRT_ENGINE_DATADIR, "setup", "dbutils")

    OVIRT_ENGINE_DB_VALIDATOR = os.path.join(OVIRT_ENGINE_DB_UTILS_DIR, "validatedb.sh")
    OVIRT_ENGINE_TASKCLEANER = os.path.join(OVIRT_ENGINE_DB_UTILS_DIR, "taskcleaner.sh")
    OVIRT_ENGINE_UNLOCK_ENTITY = os.path.join(
        OVIRT_ENGINE_DB_UTILS_DIR, "unlock_entity.sh"
    )
    ...
    OVIRT_ENGINE_SERVICE_CONFIG_KEYCLOAK = os.path.join(
        OVIRT_ENGINE_SERVICE_CONFIGD, "12-setup-keycloak.conf"
    )
    ...
    OVIRT_ENGINE_EXTENSIONS_DIR = os.path.join(OVIRT_ENGINE_SYSCONFDIR, "extensions.d")

    AAA_JDBC_DB_SCHMA_TOOL = os.path.join(
        config.AAA_JDBC_DATADIR, "dbscripts", "schema.sh"
    )

    AAA_JDBC_TOOL = os.path.join(osetupconfig.BIR_DIR, "ovirt-aaa-jdbc-tool")

    AAA_JDBC_CONFIG_DB = os.path.join(
        OVIRT_ENGINE_SYSCONFDIR, "aaa", "internal.properties"
    )
...
class Defaults(object):
    ...
    DEFAULT_CONFIG_APPLICATION_MODE = "Virt"
    ...
    DEFAULT_ADDITIONAL_PACKAGES = "ovirt-engine-ui-extensions,ovirt-web-ui"

    DEFAULT_OVN_FIREWALLD_SERVICES = "ovn-central-firewall-service,ovirt-provider-ovn"
...
def _pki_ca_uri(uri, resource):
    return ("%s/services/pki-resource?resource=%s&format=X509-PEM-CA") % (
        uri,
        resource,
    )
...
class EngineDBEnv(object):
    ...
    @osetupattrs(
        answerfile=True,
        answerfile_condition=lambda env: (
            not env.get(oengcommcons.ProvisioningEnv.POSTGRES_PROVISIONING_ENABLED)
        ),
        is_secret=True,
...
class ConfigEnv(object):
    @osetupattrs(
        ...
        summary_condition=lambda env: not env.get(CoreEnv.ENABLE),
    )
...
class OvnFileLocations(object):
    ...
    OVIRT_PROVIDER_ENGINE_SETUP_CONFIG_EXAMPLE = os.path.join(
        FileLocations.OVIRT_ENGINE_SYSCONFDIR, "ovirt-provider-ovn-conf.example"
    )
    ...
    OVIRT_PROVIDER_OVN_NDB_KEY = os.path.join(
        FileLocations.OVIRT_ENGINE_PKIKEYSDIR, OVIRT_PROVIDER_OVN_NDB + ".key.nopass"
    )
    OVIRT_PROVIDER_OVN_SDB_KEY = os.path.join(
        FileLocations.OVIRT_ENGINE_PKIKEYSDIR, OVIRT_PROVIDER_OVN_SDB + ".key.nopass"
    )
    OVIRT_PROVIDER_OVN_HTTPS_KEY = os.path.join(
        FileLocations.OVIRT_ENGINE_PKIKEYSDIR, OVIRT_PROVIDER_OVN_HTTPS + ".key.nopass"
    )

    OVIRT_PROVIDER_OVN_NDB_CERT = os.path.join(
        FileLocations.OVIRT_ENGINE_PKICERTSDIR, OVIRT_PROVIDER_OVN_NDB + ".cer"
    )
    OVIRT_PROVIDER_OVN_SDB_CERT = os.path.join(
        FileLocations.OVIRT_ENGINE_PKICERTSDIR, OVIRT_PROVIDER_OVN_SDB + ".cer"
    )
    OVIRT_PROVIDER_OVN_HTTPS_CERT = os.path.join(
        FileLocations.OVIRT_ENGINE_PKICERTSDIR, OVIRT_PROVIDER_OVN_HTTPS + ".cer"
    )

    NVMEOF_SUPPORTED = "OVESETUP_ENGINE_CONFIG/nvmeOfSupported"
