...
class Plugin(plugin.PluginBase):
    ...
    @plugin.event(
        ...
        name=ombcons.Stages.CONFIG_CINDERLIB_RENAME_COMMIT,
        condition=lambda self: self._rename_staged,
    )
    def _commit(self):
        backup_ext = datetime.datetime.now().strftime('%Y%m%d%H%M%S')
        if os.path.exists(ombcons.FileLocations.OVIRT_ENGINE_SERVICE_CONFIG_LEGACY_CINDERLIB_MISC):
            self.logger.info(
                _('Backing up and disabling legacy Cinderlib configuration file')
            )
            os.rename(
                ombcons.FileLocations.OVIRT_ENGINE_SERVICE_CONFIG_LEGACY_CINDERLIB_MISC,
                f'{ombcons.FileLocations.OVIRT_ENGINE_SERVICE_CONFIG_LEGACY_CINDERLIB_MISC}.{backup_ext}'
            )
        if os.path.exists(ombcons.FileLocations.OVIRT_ENGINE_SERVICE_CONFIG_LEGACY_CINDERLIB_DATABASE):
            self.logger.info(
                _('Backing up and disabling legacy Cinderlib database configuration file')
            )
            os.rename(
                ombcons.FileLocations.OVIRT_ENGINE_SERVICE_CONFIG_LEGACY_CINDERLIB_DATABASE,
                f'{ombcons.FileLocations.OVIRT_ENGINE_SERVICE_CONFIG_LEGACY_CINDERLIB_DATABASE}.{backup_ext}'
            )
