...
BEGIN
      ...
      cpu_per_socket = v_cpu_per_socket, threads_per_cpu = v_threads_per_cpu, os = v_os,
...
Create or replace FUNCTION GetVmTemplateByVdsGroupId(v_vds_group_id UUID) RETURNS SETOF vm_templates_based_view STABLE
   AS $procedure$
BEGIN
      RETURN QUERY SELECT vm_templates.*
      FROM vm_templates_based_view vm_templates
...
BEGIN
      ...
      WHERE vm_templates_based_view.vmt_guid = user_vm_template_permissions_view.entity_id AND
            user_vm_template_permissions_view.user_id = v_user_id AND
            (SELECT get_entity_permissions(v_user_id, v_action_group_id, vm_templates_based_view.vmt_guid, 4) IS NOT NULL);
END; $procedure$
LANGUAGE plpgsql;
