...
CREATE TYPE networkViewClusterType AS(id uuid,name VARCHAR(50),description VARCHAR(4000), free_text_comment text, type INTEGER,
	    ...
	    required BOOLEAN, migration BOOLEAN, management BOOLEAN, is_gluster BOOLEAN);
...
RETURNS VOID
   AS $procedure$
BEGIN
INSERT INTO network_cluster(cluster_id, network_id, status, is_display, required, migration, management, is_gluster)
	VALUES(v_cluster_id, v_network_id, v_status, v_is_display, v_required, v_migration, v_management, v_is_gluster);
END; $procedure$
LANGUAGE plpgsql;
...
Create or replace FUNCTION set_network_exclusively_as_gluster(v_cluster_id UUID, v_network_id UUID)
RETURNS VOID
   AS $procedure$
BEGIN
...
BEGIN
   ...
   INSERT INTO vnic_profiles(id, name, network_id, network_qos_id, port_mirroring, passthrough, custom_properties, description)
       VALUES(v_id, v_name, v_network_id, v_network_qos_id, v_port_mirroring, v_passthrough, v_custom_properties, v_description);
...
BEGIN
   ...
   port_mirroring = v_port_mirroring, passthrough = v_passthrough, custom_properties = v_custom_properties,
...
Create or replace FUNCTION GetNetworkAttachmentByNicIdAndNetworkId(v_nic_id UUID, v_network_id UUID)
RETURNS SETOF network_attachments STABLE
   AS $procedure$
BEGIN

   RETURN QUERY SELECT na.*
   FROM
    network_attachments na
   WHERE
    na.network_id = v_network_id and
    na.nic_id = v_nic_id;
