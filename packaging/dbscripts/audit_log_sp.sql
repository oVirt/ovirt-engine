...
Create or replace FUNCTION ClearAllAuditLogEvents(v_severity INTEGER)
    RETURNS VOID
AS $procedure$
BEGIN
      UPDATE audit_log SET deleted = true WHERE severity != v_severity;
END; $procedure$
LANGUAGE plpgsql;

Create or replace FUNCTION DisplayAllAuditLogEvents(v_severity INTEGER)
    RETURNS VOID
AS $procedure$
BEGIN
...
Create or replace FUNCTION SetAllAuditLogAlerts(v_severity INTEGER, v_value BOOLEAN)
    RETURNS VOID
AS $procedure$
BEGIN
      UPDATE audit_log SET deleted = v_value WHERE severity = v_severity;
END; $procedure$
LANGUAGE plpgsql;
