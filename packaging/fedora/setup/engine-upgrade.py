...
class CA():
    ...
    def prepare(self):
        ...
        if not os.path.exists(conf_file):
            conf_file = basedefs.FILE_ENGINE_CONF
        ...
        try:
            ...
            handler.close()
        if not proxyEnabled:
            logging.debug('Configuring SSL for Apache proxy')
            shutil.copyfile(
            ...
            handler.open()
            try:
                if handler.getParam("SSLCertificateFile") == '/etc/pki/tls/certs/localhost.crt':
...
def generateEngineConf():
    ...
    # RPM instead:
    logging.debug('Generating engine configuration')
    ...
    if os.path.exists(sysconfigFile):
        logging.debug(
            'Copying engine configuration in %s' % basedefs.FILE_ENGINE_CONF
        )
    ...
    try:
        ...
        handler.close()
    if proxyEnabled:
        logging.debug('oVirt engine Apache proxy was already enabled')
    else:
